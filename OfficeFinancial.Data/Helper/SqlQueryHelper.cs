...
{
    public sealed class SqlQueryHelper
    {

        public static string GetInsertQuery<T>() where T : class
        {
            var tableName = typeof(T).Name;
            PropertyInfo[] props = typeof(T).GetProperties();
            PropertyInfo[] identity = typeof(T).GetProperties().Where(p => Attribute.IsDefined(p, typeof(IdentityAttribute))).ToArray();

            string[] columns = props.Except(identity).Select(p => p.Name).Where(s => s != "ID").ToArray();
        ...
        public static string GetUpdateQuery<T>() where T : class
        {
            var tableName = typeof(T).Name;

            PropertyInfo[] keys = typeof(T).GetProperties().Where(prop => Attribute.IsDefined(prop, typeof(KeyAttribute))).ToArray();
            PropertyInfo[] props = typeof(T).GetProperties().Except(keys).ToArray();

            string[] keyColumns = keys.Select(p => p.Name).ToArray();
            string[] generalColumns = props.Select(p => p.Name).ToArray();

            var parameters = generalColumns.Select(name => name + "=@" + name).ToList();
            var condition = keyColumns.Select(name => name + "=@" + name).ToList();

            return string.Format("UPDATE {0} SET {1} WHERE {2}", tableName, string.Join(",", parameters), string.Join(" AND ", condition));
        }
        ...
        public static string GetSelectQuery<T>() where T : class
        {
            var tableName = typeof(T).Name;
        ...
        public static QueryResult GetSelectQuery<T>(Expression<Func<T, bool>> expression)
        {
            var tableName = typeof(T).Name;
            var queryProperties = new List<QueryParameter>();
            var body = (BinaryExpression)expression.Body;
